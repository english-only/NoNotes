# NoNotes + ECC (Everything Claude Code)

**NoNotes** — a focused workspace for turning study material into lasting recall — plus the **ECC** agent-harness bundle (68 agents, 286 skills, 94 legacy command shims) for Claude Code, Freebuff, Codex, OpenCode, Cursor, and related harnesses.

## NoNotes

A local-first study application built around active recall and spaced repetition: Gemini-powered flashcard generation, FSRS scheduling, Feynman evaluation, global search, analytics, and offline PWA support.
...
All ECC npm tooling is namespaced under the `ecc:` prefix (e.g. `npm run ecc:test`). Skills live canonically in `skills/` with a harness auto-load mirror in `.claude/skills/` (regenerate with `node scripts/sync-skills-to-claude.js`). See `AGENTS.md` for agent instructions.

The full ECC documentation follows.
