...
npm run test         # vitest (363 tests)
npm run test:coverage # coverage ratchet on lib/**
...
This repository also carries the **ECC (Everything Claude Code)** agent-harness bundle (68 agents, 286 skills, 94 legacy command shims) for Claude Code, Freebuff, Codex, OpenCode, Cursor, and related harnesses. All ECC npm tooling is namespaced under the `ecc:` prefix (e.g. `npm run ecc:test`). Skills live canonically in `skills/` with a harness auto-load mirror in `.claude/skills/` (regenerate with `node scripts/sync-skills-to-claude.js`). The full ECC documentation follows.
